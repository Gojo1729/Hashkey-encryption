...
class CustomerServer:
    ...
    def start_client(self, MSG):
        ...
        server_address = (
            "127.0.0.1",
            10000,
        )  # Replace 'server_ip_address' with the server's IP
    ...
    def Send_Msg_CM(self):
        ...
        if choice_1 == "M":
            ...
            # PAYLOAD
            Merchant_Payload = {
                "PAYLOAD": {"ENTITY": "Customer", "MESSAGE": "Hi Merchant"}
            }
            # payload = json.dumps(payload).encode()
            # sign=signing(payload,self.customer1_private_key)
            Merchant_Encrypted_Payload = encrypt_data(
                Merchant_Payload, self.merchant_public_key
            )
            payload = {
            ...
            # sign=signing(payload,self.customer1_private_key)
            payload = json.dumps(payload).encode("latin1")
            encrypted_data = encrypt_data(payload, self.broker_public_key)
            print("Message Sent (Encrypted Format): ", encrypted_data)
            print("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
            self.start_client(encrypted_data)
